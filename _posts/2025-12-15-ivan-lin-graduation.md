---
layout: post
title: Congratulations Dr Jiancheng (Ivan) Lin!
date: 2025-12-15
description: Ivan successfully defended his PhD thesis — the first PhD graduate from the Chi Lab!
tags: graduation people
categories: lab-life
thumbnail: assets/img/ivan_graduation_1.jpg
images:
  slider: true
---

We are incredibly proud to celebrate **Dr Jiancheng (Ivan) Lin**, who successfully defended his PhD thesis on **15 December 2025** — becoming the **first PhD graduate from the Chi Lab**! 🎉🎓

Ivan's thesis, *"Controlled electrocrystallization of metal-organic charge transfer complexes for gas sensor integration"*, represents outstanding and creative work at the intersection of electrochemistry, materials science, and sensing technologies.

Ivan joined the Chi Lab in his final year after transferring from Prof Mao's group, and in that time he made a tremendous impression on everyone in the lab. He brought focus, dedication, and a genuine spirit of collaboration that made our group better. It was a privilege to be part of his final chapter as a PhD student.

We wish Ivan all the very best in the next exciting chapter of his journey — the best is yet to come! Once a Chi Lab member, always a Chi Lab member. 🎉

---

*Congratulations, Dr Lin — we are so proud of you!*

---

## Celebrating Ivan

<swiper-container keyboard="true" navigation="true" pagination="true" pagination-clickable="true" pagination-dynamic-bullets="true" rewind="true">
  <swiper-slide>{% include figure.liquid loading="eager" path="assets/img/ivan_graduation_1.jpg" class="img-fluid rounded z-depth-1" %}</swiper-slide>
  <swiper-slide>{% include figure.liquid loading="eager" path="assets/img/ivan_graduation_2.jpg" class="img-fluid rounded z-depth-1" %}</swiper-slide>
  <swiper-slide>{% include figure.liquid loading="eager" path="assets/img/ivan_graduation_3.jpg" class="img-fluid rounded z-depth-1" %}</swiper-slide>
</swiper-container>
